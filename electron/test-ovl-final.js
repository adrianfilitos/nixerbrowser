...
process.env.NIXER_USER_DATA = path.join(os.tmpdir(), 'nixer-ovl-final-test')
...
app.whenReady().then(async () => {
  ...
  await delay(1000)
  const server = http.createServer((req, res) => { res.setHeader('Content-Type', 'text/html'); res.end('<html><body style="background:#ffffff"><h1>WHITE PAGE</h1></body></html>') })
  await new Promise((r) => server.listen(0, '127.0.0.1', r))
  const port = server.address().port
  await ui.executeJavaScript(`window.api.tabLoad('${tabId}', 'http://127.0.0.1:${port}/')`)
  await delay(3000)
  console.log('WHITE_LOADED')
  win.show(); win.focus()
  ...
  await ui.executeJavaScript(`document.querySelector('.menu-btn').click(); true`)
  await delay(800)
  console.log('CYCLE_DONE_HWND=' + win.getNativeWindowHandle().readInt32LE(0))
